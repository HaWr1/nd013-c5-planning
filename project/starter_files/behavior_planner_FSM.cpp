...
constexpr double COMFORTABLE_DECELERATION =
    -4.0; // comfortable deceleration in m/s²
...
double BehaviorPlannerFSM::get_look_ahead_distance(const State &ego_state) {
  ...
  auto look_ahead_distance =
      velocity_mag * velocity_mag / (2 * accel_mag); // ! FIXED
  // auto look_ahead_distance = velocity_mag * velocity_mag /
  //                            (2 * COMFORTABLE_DECELERATION); // TODO: CHECK
...
State BehaviorPlannerFSM::state_transition(const State &ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    auto distance_to_stop_sign =
        utils::magnitude(goal.location - ego_state.location); // ! FIXED
    ...
    if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) { // ! FIXED
